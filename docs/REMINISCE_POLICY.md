# Reminisce 정책 (프로젝트 규칙)

이 문서는 `reminisce` 프로젝트가 따를 아키텍처·레이어·정책을 정의합니다.
...
- **프로젝트 패키지 prefix**는 **한 가지로 통일**한다 (예: `com.krince.reminisce`). 다른 프로젝트 이름(예: prizm)이 코드에 남지 않도록 한다.
...
1. 패키지 prefix를 새 프로젝트명으로 일괄 변경 (예: `com.krince.reminisce` → `com.krince.새프로젝트`).
